...
movies$image_url = sapply(movies$MovieID, 
                          function(x) paste0(small_image_url, x, '.jpg?raw=true'))genres = as.data.frame(movies$Genres, stringsAsFactors=FALSE)
...
```{r}
movies_filtered = movies["Action" %in% tmp,]
...
S = proxy::simil(t(trains), method = "cosine")
mypred = rep(NA, nrow(S))
for (i in 1:nrow(S)) {
  S_i = S[i,]
  S_i[i] = 0
  S_filtered = S_i[S_i != 0 & !is.na(S_i)]
